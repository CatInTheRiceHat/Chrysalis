...
and no item published. The owner’s storage decision is implemented: password-free viewing, temporary activity by default, optional encrypted history. See `../../docs/encrypted-history-storage.md`. Publisher certification and a publicly accessible matching 0.9.0 policy remain required; no approval is guaranteed.
...
| Privacy URL | https://thechrysalisproject.vercel.app/privacy — existing public destination currently serves the earlier policy. Publish the matching 0.9.0 policy at a stable public HTTPS destination before submission; the protected launch preview is insufficient. Do not replace production in this task. |
...
| Local-only data handling still needs disclosure and privacy policy | Full local policy and public policy URL verified; optional history is encrypted and temporary activity stays in memory; final publisher certification and public 0.9.0 policy remain. GitHub Issues is verified and the repository email is documented. | [User Data FAQ](https://developer.chrome.com/docs/webstore/program-policies/user-data-faq) |
...
**Prepared materials:** `release/chrysalis-0.9.0-store-preparation/` is assembled by `node scripts/prepare-store.mjs` after package validation and current screenshots. It includes the exact upload ZIP/checksum, five release screenshots, icon, small promo, listing, disclosures and identity guidance. Review the inventory and validation for actual evidence. The public 0.9.0 policy and publisher account setup must be completed before submission. No Store URL or publisher identity is invented.
