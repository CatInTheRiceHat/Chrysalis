...
try {
 ...
 const popup=await context.newPage();await popup.goto(`chrome-extension://${id}/popup.html`);await popup.locator('#intro-skip').click();await popup.evaluate(async()=>{const r=await chrome.runtime.sendMessage({channel:'chrysalis/v1',type:'GET_SETTINGS'});await chrome.runtime.sendMessage({channel:'chrysalis/v1',type:'UPDATE_SETTINGS',expectedRevision:r.revision,patch:{indicatorCollapsed:false}});});await popup.locator('#intention').selectOption('Exploring');await popup.locator('#submit-plan').click();
 ...
 const i=await indicator.boundingBox(),p=await page.locator('#movie_player').boundingBox();assert(p&&i&&i.x>=0&&i.x+i.width<=1440);
 assert.equal(await indicator.evaluate(e=>e.parentElement===document.documentElement),true);
 ...
 await indicator.getByRole('button',{name:'Pause session',exact:true}).click();await expect(indicator.getByRole('button',{name:'Resume session',exact:true})).toBeVisible();
 ...
 checks.push('Live watch timer floats within viewport bounds; current intention/foreground time, Pause and collapse work.');
 ...
 checks.push('Live Home timer stays below the masthead; YouTube page content retained.');
