...
const suites = ['browser', 'session-browser', 'viewing-browser', 'experience-browser', 'checkpoints-browser', 'history-browser', 'hardening-browser', 'usability-browser', 'usability-native'];
for (const suite of suites) {
  console.log(`Packaged-extension verification: ${suite}`);
  const result = spawnSync(process.execPath, [`tests/${suite}.mjs`], { stdio: 'inherit', env: { ...process.env, CHRYSALIS_EXTENSION_PATH: unpacked } });
  assert.equal(result.status, 0, `Packaged extension failed ${suite}`);
}
await writeFile(`${stem}-validation.json`, JSON.stringify({ version, zipSha256: report.sha256, verifiedAt: new Date().toISOString(), unpacked, passedSuites: suites, evidence: 'Real unpacked extension loaded from ZIP-extracted assets in disposable Chromium profiles. Controlled YouTube-origin fixtures; live selector evidence is separate.' }, null, 2) + '\n');
console.log('All nine suites passed against the extracted distribution ZIP.');
