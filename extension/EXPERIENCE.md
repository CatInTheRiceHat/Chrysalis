...
**0.9.2 update: all newly started/edited sessions require a time target. The introduction no longer offers untimed continuation. Minutes are directly editable alongside presets in the introduction, popup and default preferences. Close/Escape remain available. Tab activation now refreshes the selected timer even if renderer focus events are delayed or absent.**
