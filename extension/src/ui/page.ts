...
intro.innerHTML = `<p class="eyebrow">Welcome to Chrysalis</p><h1>Your time.<br>Your choice.</h1>
<p class="intro">Make a YouTube session match what you came for—studying, a particular video, entertainment or a little exploring.</p>
...
<p class="note">For <strong>www.youtube.com in desktop Chrome</strong>. Settings and session summaries stay on this device, with no account or analytics. Your intention appears in the YouTube indicator; avoid private details. You can hide it in settings.</p>
<button id="intro-start" class="primary">Get started</button><button id="intro-skip" class="text-button full-width">Skip introduction</button><p id="intro-status" class="status" role="status"></p>`;
const checks = [
...
function status(message: string, error = false) {
  ...
  for (const id of ['retry', 'viewing-retry']) { const el = $(id); if (el) el.hidden = !error; }
}
...
function render(settings: Settings, nextRevision: number) {
  ...
  if (target && document.activeElement !== target && document.activeElement !== $('default-custom')) {
    ...
    if (value !== 'none') $<HTMLInputElement>('default-custom')!.value = value;
  }
  const duration = $<HTMLInputElement>('break-preference');
  if (duration && document.activeElement !== duration) duration.value = String(settings.breakMinutes);
}
...
for (const id of ['retry', 'viewing-retry']) $(id)?.addEventListener('click', () => void load());
for (const id of ['intro-start', 'intro-skip']) $(id)?.addEventListener('click', async () => {
  if (await save({ introSeen: true })) ($('intention') ?? document.querySelector<HTMLElement>('main h1'))?.focus();
});
...
const dialog = document.createElement('dialog'); dialog.setAttribute('aria-labelledby', 'delete-title');
