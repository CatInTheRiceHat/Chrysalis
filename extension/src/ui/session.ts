...
export function mountSession(root: HTMLElement) {
  ...
  root.innerHTML = `
    ...
    <form id="session-plan" hidden>
      ...
      <p class="note">A target is a plan you can revise, not a limit. Changing the total target arms a new checkpoint; changing only your intention does not.</p>
  ...
  function render(state: StorageSnapshot) {
    ...
    if (s.phase !== 'idle') {
      ...
      if (s.phase === 'finished') {
        ...
        for (const [name, value] of [['Original target', targetText(s.originalTargetMs)], ['Final target', targetText(s.targetMs)], ['Foreground YouTube time', clockText(s.elapsedMs)], ['Break time (wall-clock)', !s.history.complete && !s.history.breakMs ? 'Not recorded in the earlier version' : clockText(s.history.breakMs)]]) {
    ...
    resume.textContent = s.phase === 'break' ? 'Resume session now' : 'Resume';
